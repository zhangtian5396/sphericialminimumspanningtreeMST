...
D=model.n-2;             % Number of Decision Variables维度是节点数-2
lb = 1;                  %下界为1
ub = model.n;            %上界为节点数
%------------------------------------------------------------------------------------
%随机初始化粒子种群.
...
%评价每个粒子的适应度值. 
    ...
    if iteration==1
       Fbest=best;Lbest=X(best_X,:);%全局最优及其位置
    end
    if tag==1
...
BestValues=[BestValues Fbest];%储存每一代的最优值
bestsol = sol(best_X);%储存最优个体的生成树信息
MeanValues=[MeanValues mean(fitness)];%储存每一代的目标函数值的平均值
%-----------------------------------------------------------------------------------
% 电荷 
...
if Fmax==Fmin
   ...
   Q=exp((fitness-worst)./(best-worst)); %计算q
end

Q=Q./sum(Q); %计算Q，粒子的电荷量
%----------------------------------------------------------------------------------
...
alfa=30;
K0=500;
K=K0*exp(-alfa*iteration/max_it);%库伦常数
%---------------------------------------------------------------------------------- 
%计算加速度.
a=E*K; 
%----------------------------------------------------------------------------------
...
if size(up,2)==1%如果up的列数等于1
    X=rand(N,dim).*(up-down)+down;%若种群个数为30，节点数为25，则初始化种群为30*23
end
if size(up,2)>1%如果up的列数大于1
    for i=1:dim
